...
to go
  ...
  ask turtles [
    ...
    if social-behaviour = "conformers"[
...
to turtle-talk
  if any? turtles at-points vision-points [
    ...
    let sender self
    if abs(epsilon - [epsilon] of receiver) <= theta-value [
...
to adjust-thresholds
  carefully [
    ...
    let y count other turtles at-points vision-points with [not member? who my-memory]
    ifelse x > 0 [
    set threshold-1 y / x
  ]
  [set threshold-1 0]
  ...
  if resources-redistribution = true [
    if member? COUNTER PAYOFF-TICKS [
      ...
      if length ACCUMULATED-STORAGE > 1 [
       ...
       foreach increments [ x ->
          if STORAGE > x and STORAGE - second-last >= increments-for-redistribution [
            ask patches [
              set max-psugar max-psugar + sugar-increment
              set incremented-psugar lput sugar-increment incremented-psugar
              foreach incremented-psugar [ inc ->
                let total-increment (list(last incremented-psugar))
...
to color-agents-by-norms
  ...
  if group != "none" and group != "defectors" [
    set color blue
  ]
end
@#$#@#$#@
...
1165
480
Cooperators-groups-distribution
NIL
NIL
0.0
10.0
...
365
Cooperators
count turtles with [group != \"defectors\" and group != \"none\"]
17
1
11
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot  sum [max-psugar] of patches"
...
@#$#@#$#@
<experiments>
  <experiment name="experiment-norms" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    ...
    <metric>count turtles with [group = "defectors"]</metric>
    <metric>count turtles with [group != "defectors" and group != "none"]</metric>
  ...
  <experiment name="experiment-no- norms" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>count turtles</metric>
